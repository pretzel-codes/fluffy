...
export function PricingPlan() {
  return (
    <section className="space-y-14 py-24 md:py-32">
            ...
            <article
              ...
              style={{
                boxShadow: plan.featured
                  ? 'rgba(16, 65, 176, 0.3) 0px 4px 12px'
                  : 'rgba(0, 0, 0, 0.1) 0px 4px 12px',
              }}
            >
              <div className="flex items-center gap-2">
                <h2 className="card-title">{plan.name}</h2>
                {plan.featured && <Sparkle className="fill-primary size-3.5" />}
              </div>

              <p className="mt-1 text-xl font-bold md:text-3xl">{plan.price}</p>

              <p className="card-description mt-4">{plan.description}</p>
